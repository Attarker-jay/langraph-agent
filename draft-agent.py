...
document_content = "" # Global variable to store document content
...
tools = [update, save]  # Tools created

model = ChatOpenAI(model="gpt-4o").bind_tools(dfsdfsdfsd)
...
graph.add_node("tools", ToolNode(tools))  # Adding tools node
...
graph.add_conditional_edges(
    ...
    should_continue,
    {
        "continue": "our_agent",  # If the conversation should continue, go back to our_agent
        "end": END,  # If the conversation should end, go to END node
    },
)
